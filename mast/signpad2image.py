...
def s2i(jsonsig, input_image=BLANK_IMAGE, pincolor=(0,0,0),
...
def test(imgname='testsig.png'):
    
    try:
        #jsonsig =  sys.argv[1]
        output_image =  imgname
        image_path=s2if(j, output_image)
        return "New signature image written to: %s " % (image_path)
    except:
        return sys.exc_info()
